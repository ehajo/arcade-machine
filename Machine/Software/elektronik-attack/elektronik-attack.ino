...
#define ALIEN_TYPES ENEMY_SPRITE_COUNT

Adafruit_SSD1306 display(SCREEN_WIDTH, SCREEN_HEIGHT, &SPI, OLED_DC, OLED_RST, OLED_CS);

// ====================== ELECTRONICS ATTACK SPRITE PACK ======================
// 8x8 schematic-symbol enemies

const uint8_t SPR_RESISTOR[] PROGMEM = {
0b00000000,
0b00111100,
0b11100111,
0b00111100,
...
const uint8_t SPR_CAPACITOR[] PROGMEM = {
0b00100100,
0b00100100,
0b11100111,
0b00100100,
...
const uint8_t SPR_DIODE[] PROGMEM = {
0b01000100,
0b01110100,
0b11111111,
0b01110100,
...
const uint8_t SPR_COIL[] PROGMEM = {
0b00000000,
0b00111100,
0b11111111,
0b00111100,
...
// player soldering iron
const uint8_t SPR_SOLDERING_IRON[] PROGMEM = {
0b11111100,
0b11111110,
0b11111110,
0b00011111,
...
// player projectile (solder)
const uint8_t SPR_SOLDER[] PROGMEM = {
0b00011000,
0b00011000,
0b00011000,
0b00011000
...
// enemy projectile (electron)
const uint8_t SPR_ELECTRON[] PROGMEM = {
0b00111100,
0b00100100,
0b00111100
};

const uint8_t* ENEMY_SPRITES[] = {
SPR_RESISTOR,
SPR_CAPACITOR,
SPR_DIODE,
SPR_COIL,
...
const uint8_t ENEMY_SPRITE_COUNT = 5;
...
// Elektronik-Bauteile: nur 2 Reihen, mehr Abstand
static const int ALIEN_COLS = 7;
static const int ALIEN_ROWS = 2;

static const int ALIEN_W = 8;
static const int ALIEN_H = 5;
...
static void resetAliens(){
  ...
  // pro Level zufaellige Bauteil-Typen je Zeile (und moeglichst nicht doppelt)
  for(int r=0;r<ALIEN_ROWS;r++){
    uint8_t t = (uint8_t)random(ENEMY_SPRITE_COUNT);
    if(r>0 && ENEMY_SPRITE_COUNT>1){
...
static void drawShip(uint32_t now){
  ...
  // 9x4 Loetkolben-Sprite
  static const uint16_t solderingIron[SHIP_H] = {
    0b111111100, // Griff
    0b111111110, // Griff + Schaft
    0b001111111, // Schaft + Spitze
    0b000011110  // feine Spitze
  };

  for(int yy=0; yy<SHIP_H; yy++){
    uint16_t row = solderingIron[yy];
    for(int xx=0; xx<SHIP_W; xx++){
      if(row & (1U << (SHIP_W - 1 - xx))){
        display.drawPixel(gs.shipX + xx, gs.shipY + yy, SSD1306_WHITE);
      }
    }
  }

  // kleines Glimmen an der Spitze
  if(((now/180) & 1) == 0){
    display.drawPixel(gs.shipX + SHIP_W - 1, gs.shipY + 2, SSD1306_WHITE);
  }
}

static void drawAliens(){
  for(int r=0;r<ALIEN_ROWS;r++){
    uint8_t t = rowAlienType[r] % ENEMY_SPRITE_COUNT;
    ...
    for(int c=0;c<ALIEN_COLS;c++){
      ...
      const uint8_t* sprite = ENEMY_SPRITES[t % ENEMY_SPRITE_COUNT];
...
static void drawStartScreen(uint32_t now){
  display.setTextSize(1);
  display.setTextColor(SSD1306_WHITE);
  display.setCursor(16, 18); display.print("Elektronik");
  display.setCursor(34, 28); display.print("Attack");
...
void setup(){
  ...
  Serial.println("[ARCADE] Elektronik Attack master boot");
